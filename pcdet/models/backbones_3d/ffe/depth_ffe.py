...
class DepthFFE(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        depth_maps = batch_dict["depth_maps"]
        depth_target = depth_utils.bin_depths(depth_maps, **self.disc_cfg, target=True)
        B, H, W = depth_target.size()
        depth_logits = torch.zeros(B, H, W, 80).to(depth_target.device).scatter(3, depth_target.unsqueeze(-1), 1).permute(0, 3, 1, 2)       
    ...
    def create_frustum_features(self, image_features, depth_logits):
        ...
        frustum_features = depth_logits * image_features
